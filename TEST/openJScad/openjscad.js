//var SSPAN;
function menuline() {
document.write('<button class="mbutton" type="button" onclick="top.location.href=\'http://www.3ddoctors.net\'">Object Library</button>');
document.write('<button class="mbutton" type="button" onclick="top.location.href=\'http://www.3ddoctors.net/\custom.html\'">Custom-made template</button>');
//document.write('<button type="button" onclick="top.location.href=\'http://3ddoctors.comyr.com/\whatelse.html\'">What Else Do We Do</button>');
document.write('<button class="mbutton" type="button" onclick="top.location.href=\'http://www.3ddoctors.net/\aboutus.html\'">About Us</button>');
}

function toImage() {  //OFER
    var dataURL = gProcessor.viewer.gl.canvas.toDataURL("image/png");

      dataURL = dataURL.replace("image/png", "image/octet-stream");
      //document.location.download="IMAGE.png";
...
OpenJsCad.Viewer = function(containerelement, width, height, initialdepth) {
  ...
  gl.clearColor(0.2, 0.2263, 0.2355, 1); //CLR background BBCCFF
  ...
  gl.onmousewheel = function(e) {
    ...
    if(wheelDelta)
    {
      var factor = Math.pow(1.002, -wheelDelta);
...
OpenJsCad.Viewer.prototype = {
  setCsg: function(csg) {
    this.meshes = OpenJsCad.Viewer.csgToMeshes(csg); //1234
    this.onDraw();
  },
  ...
  onMouseMove: function(e) {
    if (e.dragging) {
      this.drawAxes=true;
      e.preventDefault();
      if((e.altKey)&&(e.shiftKey)) { //ROTATE Z, X
	    ...
	    //
	    //save image:
        if (this.saved==false) {toImage(); this.saved=true;}
        this.gl.clearColor(0.73, 0.8, 1, 1);
  ...
  onDraw: function(e) {
    ...
    if (this.drawAxes) {
      ...
      gl.begin(gl.LINES);
      //X - red
      gl.color(0.7, 0, 0, 0.8); //negative direction is lighter
      gl.vertex(-100, 0, 0);
      gl.vertex(0, 0, 0);

      gl.color(0.7, 0, 0, 0.8); //positive direction
      gl.vertex(0, 0, 0);
      gl.vertex(100, 0, 0);
      //Y - green
      gl.color(0.7, 0, 0, 0.8); //negative direction is lighter
      ...
      // BEGIN PLANE Z=0
       ...
       for (var ii=-100;ii<100;ii+=1) {
         if (ii%10==0) {ii+=1;}
         gl.color(0.00, 0.00, 0.2, 0.15)
         gl.vertex(ii, -100, 0);
         gl.vertex(ii, 100, 0);

	 //gl.color(0.63, 0.72, 0.9, 1)
	gl.color(0.00, 0.00, 0.2, 0.15)
         gl.vertex(-100,ii, 0);
         gl.vertex(100,ii, 0);
	}

       for (var ii=-100;ii<=100;ii+=10) {
         if (ii==0) {ii+=10;}
         gl.color(0, 0, 0, 1)
         gl.vertex(ii, -100, 0);
         gl.vertex(ii, 100, 0);
...
OpenJsCad.Viewer.csgToMeshes = function(csg) {
  ...
  for(var polygonindex = 0; polygonindex < numpolygons; polygonindex++)
  {
    ...
    //var color = [1,0.2,0.1]; // CLR
    //var color = [1,0.3,0.1]; // the red
    var color = [1,0.9,0.6];  //FFE69A
...
OpenJsCad.runMainInWorker = function(mainParameters)
{
  ...
  catch(e)
  {
    ...
    self.postMessage({cmd: 'error', err: errtxt}); //'Error: Check your parameters!'
  }
};
...
*/
    ...
    viewerdiv.style.backgroundColor = "rgb(255,255,204)";
    ...
    this.zoomControl = div.cloneNode(false);  // SCROLL
    this.zoomControl.style.width = this.viewerwidth + 'px';
    this.zoomControl.style.height = '0px';  //used to be 20
    ...
    this.statusspan = document.createElement("span"); // Processing
    //this.selectspan = document.createElement("span");
    this.statusspan2 = document.createElement("span"); // Dimensions
    //this.statusbuttons = document.createElement("div");

    this.statusspan.style.left='5px';
    this.statusspan.style.top='-305px';
    this.statusspan.style.padding='0px';
    this.statusspan.style.position='relative';
    ...
    //this.selectspan.appendChild(this.renderedElementDropdown); // used to be statusbuttons
    ...
    //this.statusbuttons.appendChild(this.updateButton);

    this.sharebutton = document.createElement("button");
    this.sharebutton.innerHTML = '<img src="..\/..\/icons\/share5.png" height=18 align="top"> Save/Share Model </img>';

    this.sharebutton.onclick=function(e) {
      showpopup();
    };
    //this.statusbuttons.appendChild(this.sharebutton);

    //this.statusdiv.appendChild(this.statusbuttons);

    this.downtable = document.createElement('table');

    //this.downtable.className="downtable";
    ...
    this.td21=document.createElement('td');    this.td21.setAttribute("colspan","2");
    this.td22=document.createElement('td');    this.td22.setAttribute("colspan","2");
    this.td31=document.createElement('td');    this.td31.setAttribute("colspan","3");
    this.td32=document.createElement('td');    this.td32.setAttribute("colspan","3");

    this.viewerdiv.appendChild(this.statusspan);

    this.td21.appendChild(this.updateButton);
    this.td22.appendChild(this.abortbutton); this.td22.appendChild(this.generateOutputFileButton);this.td22.appendChild(this.renderedElementDropdown);
    this.td31.appendChild(this.downloadOutputFileLink);
    this.td32.appendChild(this.sharebutton);



    this.downtable.appendChild(this.td21); this.downtable.appendChild(this.td22);
    //this.downtable2 = document.createElement('table');
    //this.downtable2.appendChild(this.td31); this.downtable2.appendChild(this.td32);
    this.downtable.appendChild(this.tr3);
    this.downtable.appendChild(this.td31); this.downtable.appendChild(this.td32);

    //this.statusdiv.appendChild(this.downtable);  //this.statusdiv.appendChild(this.downtable2);
    //this.statusdiv.appendChild(this.statusspan2); // dimensions

    this.containerdiv.appendChild(this.downtable);
    this.containerdiv.appendChild(this.statusspan2);
  ...
  setRenderedObjects: function(obj) {
    ...
    for(var i=0; i < obj.length; ++i)
    {
      ...
      if (caption!="Illustration")
		{
			if (i!=0) {sstemp+="<br>";}
			if (obj.length>1) { sstemp+=caption+ " dimensions: ";  } else {sstemp+="Dimensions: "; }

			sstemp+=(Math.round(100*(renderedelement.data.getBounds()[1].x-renderedelement.data.getBounds()[0].x)) / 100).toString()+" x ";
			sstemp+=(Math.round(100*(renderedelement.data.getBounds()[1].y-renderedelement.data.getBounds()[0].y)) / 100).toString();

			 if (typeof(renderedelement.data.getBounds()[1].z) !== 'undefined')  {
				 sstemp+=" x ";
			     sstemp+=(Math.round(100*(renderedelement.data.getBounds()[1].z-renderedelement.data.getBounds()[0].z)) / 100).toString();
		  }
			this.statusspan2.innerHTML=sstemp;
  ...
  updateDownloadLink: function() {
    ...
    this.generateOutputFileButton.innerHTML = "Generate "+ext.toUpperCase();  // 1234
  },
  ...
  enableItems: function() {
    ...
    this.generateOutputFileButton.style.display =this.processing? "none":"inline"; //((!this.hasOutputFile)&&(this.hasValidCurrentObject))? "inline":"none";
    this.generateOutputFileButton.disabled =((!this.hasOutputFile)&&(this.hasValidCurrentObject))? false:true;
    //this.generateOutputFileButton.style.opacity = ((!this.hasOutputFile)&&(this.hasValidCurrentObject))? "1":"0.8";
  ...
  setJsCad: function(filename) {
   ...
   // }
    if(!scripthaserrors)
    {
      this.script = "function main() {return new CSG() ;}";
  ...
  */
  rebuildSolid: function()
  {
    ...
    if(!useSync)
    {
	  ...
	  that.statusspan2.innerHTML = "";
        }
        else
        {
          that.setRenderedObjects(main());
          that.statusspan.innerHTML = ""; // used to be "Ready."
        }
        that.enableItems();
  ...
  downloadLinkTextForCurrentObject: function() {
    var ext = this.selectedFormatInfo().extension;
    return '<img src="..\/..\/icons\/down5.png" height=18 align="top"> Download '+ext.toUpperCase()+'</img>';
  },
  ...
  generateOutputFileBlobUrl: function() {
    ...
    var ext = this.selectedFormatInfo().extension;
    this.downloadOutputFileLink.setAttribute("download", "openjscad."+ext); // Show Download link
//    this.downloadOutputFileLink.click();  works only in firefox
    this.enableItems();
    if(this.onchange) this.onchange();
  },
