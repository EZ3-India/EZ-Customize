//var SSPAN;
function menuline() {
document.write('<button type="button" onclick="top.location.href=\'http://3ddoctors.comyr.com\'">Object Library</button>');
document.write('<button type="button" onclick="top.location.href=\'http://3ddoctors.comyr.com\'">Can\'t find what you need?</button>');
...
document.write('<button type="button" onclick="top.location.href=\'http://3ddoctors.comyr.com/\aboutus.html\'">About Us</button>');
}
...
function PutExamples(srclink,howmany,cols,wid) {
	for(var N=1;N<=howmany;N++) {
		document.write('<a href="#"><img src="'+srclink+N.toString()+'.png" width='+wid.toString()+' onclick="EX('+N.toString()+')"></a>');
...
OpenJsCad.Viewer = function(containerelement, width, height, initialdepth) {
  ...
  gl.clearColor(0.73, 0.8, 1, 1); //CLR background BBCCFF
...
*/    
    ...
    this.statusspan2 = document.createElement("span");
    //SSPAN=this.statusspan2;
    ...
    this.statusdiv.appendChild(this.statusspan2);
  ...
  setRenderedObjects: function(obj) {
    ...
    for(var i=0; i < obj.length; ++i)
    {
      ...
      if (caption!="Illustration") 
		{ 
			if (obj.length>1) { sstemp+="<br>"+caption+ " dimensions: ";  } else {sstemp+="<br> Dimensions: "; }


			sstemp+=(Math.round(100*(renderedelement.data.getBounds()[1].x-renderedelement.data.getBounds()[0].x)) / 100).toString()+" x ";
			sstemp+=(Math.round(100*(renderedelement.data.getBounds()[1].y-renderedelement.data.getBounds()[0].y)) / 100).toString()+" x ";
