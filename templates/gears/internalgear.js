...
{  
  ...
  var OuterDiameter = PitchDiameter+(2*Module*1.25);//2*Module;
  var RootDiameter = PitchDiameter-(2*Module*0.5);
  ...
  var ExD=OuterDiameter+10;
  // creating the 2D shape - start with involute
  var gear=createGear(PitchDiameter,RootDiameter,OuterDiameter,ExD,PressureAngle,N,H1);
  //var gear=new CSG();
...
function createGear(PitchDiameter,RootDiameter,OuterDiameter,ExD,PressureAngle,N,H1) {
  ...
  var Excircle= new CAG.circle({center: [0, 0], radius: ExD/2 ,resolution:72});
  ...
  gear=Excircle.subtract(RDcircle).extrude({offset: [0, 0, H1]}).subtract(gear);
